...
int uniqueFactoryId;
int uniqueCarId;
EditDialog::EditDialog(QSqlRelationalTableModel *cars, QSqlTableModel *factory, QDomDocument details, QFile *output, QWidget *parent) : QDialog(parent)
{
    carModel = cars;
...
void EditDialog::submit()
{
    ...
    if (factory.isEmpty() || address.isEmpty() || name.isEmpty()) {
        QMessageBox::information(this, tr("添加产品"), tr("请输入厂名、厂址和商品名称"));
    } else {
...
QGroupBox* EditDialog::createInputWidgets()
{
    ...
    QLabel *attribLabel = new QLabel(tr("产品属性(使用分号;隔开):"));
